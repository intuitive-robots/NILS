...
from nils.segmentator.deva_consensus_automatic import (
    find_consensus_auto_association,
)
...
class DEVAInferenceCore:
    ...
    def check_for_resegmentation(self):
        #Check all annotated masks in memory. If a mask has multiple components, we should resegment.

        class_to_obj = self.object_manager.class_to_obj_id

        for class_id, obj in class_to_obj.items():
            mask_ids = np.array([self.object_manager.obj_to_tmp_id[obj_id] for obj_id in obj])

            mask_merged = self.last_mask_bool[mask_ids -1].sum(dim=0).cpu().numpy()

            mask_merged_dilated  = cv2.dilate(mask_merged.astype(np.uint8), np.ones((5,5),np.uint8), iterations = 8)



            n_components,labels,stats,centroids = cv2.connectedComponentsWithStats(mask_merged_dilated.astype(np.uint8))
            ...
            n_components
            #Dont compare masks, instead do contours (more efficient)
            if n_components > 2:
                # contours = [cv2.findContours((labels == i).astype(np.uint8), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[1] for i in range(1, n_components)]
                #
                # min_dists = []
                # for i in range(len(contours)):
                #     c = np.unique(contours[i].astype(int), axis=0)
                #     for j in range(len(contours)):
                #         if i == j:
                #             continue
                #     else:
                #         other_contour = np.unique(contours[j].astype(int), axis=0)
                #         dists = np.linalg.norm(c - other_contour, axis=1)
                #         if np.min(dists) > 20:
                return True
    ...
    def step(self,
        ...
        self.last_mask_bool = torch.argmax(pred_prob_with_bg[1:], dim=0)
        self.last_mask_bool = self.object_manager.make_one_hot_temp_id(self.last_mask_bool)
