...
def merge_by_iou(our_masks: Dict[ObjectInfo, torch.Tensor], new_masks: Dict[ObjectInfo,
    ...
    for our_obj in object_manager.obj_to_tmp_id:
        ...
        if len(our_obj.category_ids) == 0:
            continue
        cur_obj_class  = max(our_obj.category_ids, key = our_obj.category_ids.count)

        if (our_obj.isthing != isthing_status) or (our_obj in our_to_new_matching):
            if cur_obj_class not in class_id_match_dict:
                class_id_match_dict[cur_obj_class] = [0]
            else:
                class_id_match_dict[cur_obj_class].append(0)
            continue
        matched_area[(our_obj, False)] = our_sums[our_obj]
        our_obj.missed_merges += 1
        #our_obj.scores = [s -0.02 for s in our_obj.scores]
        #our_obj.class_scores = [s -0.02 for s in our_obj.class_scores]
        if cur_obj_class not in class_id_match_dict:
            class_id_match_dict[cur_obj_class] = [1]
        else:
            class_id_match_dict[cur_obj_class].append(1)



        # try:
        #     plt.imshow(our_masks[our_obj].cpu().numpy())
        #     plt.show()
        # except:
        #     pass
    ...
    return merged_mask,class_id_match_dict
...
def match_and_merge(our_mask: torch.Tensor,
    ...
    match_isthing = [None]  # for isthing
    # we merge stuff/things/others separately
    for isthing_status in match_isthing:
        if mode == 'iou':
            merged_mask,class_id_match_dict = merge_by_iou(our_masks, new_masks, our_sums, new_sums, merged_mask,
