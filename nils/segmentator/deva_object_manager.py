...
class ObjectManager:
    ...
    def __init__(self):
        ...
        self.unfiltered_obj_to_tmp_id: Dict[ObjectInfo, int] = {}

        self.class_to_obj_id: Dict[int, List[ObjectInfo]] = {}

        #also create a permanent mask to temp id mapping
        self.tmp_id_to_mask_final_id = {}
    ...
    def calc_class_to_obj_id(self) -> None:
        self.class_to_obj_id = {}
        for obj in self.obj_to_tmp_id:
            if obj.vote_category_id() is not None:
                if obj.vote_category_id() not in self.class_to_obj_id:
                    self.class_to_obj_id[obj.vote_category_id()] = []
                self.class_to_obj_id[obj.vote_category_id()].append(obj)
    ...
    def make_one_hot_temp_id(self, cls_mask: torch.Tensor) -> torch.Tensor:
        output = []
        for tmp_id, obj in self.tmp_id_to_obj.items():
            output.append(cls_mask == (tmp_id -1)) #tmp id starts at 1, cls_mask doesnt have bg so -1
        if len(output) == 0:
            output = torch.zeros((0, *cls_mask.shape), dtype=torch.bool, device=cls_mask.device)
        else:
            output = torch.stack(output, dim=0)
    ...
    def get_current_segments_info(self) -> List[Dict]:
        segments_info = []
        for obj in self.obj_to_tmp_id:
            if len(obj.category_ids) == 0:
                continue
            segments_info.append({
                'category_id': obj.vote_category_id_scored(),
                'id': int(obj.id),
