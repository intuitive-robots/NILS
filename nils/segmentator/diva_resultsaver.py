...
class ResultSaver:
    def __init__(self,
        ...
        self.class_mask_dict_postfix = "ClassMaskDict"
    ...
    def save_mask(self,
        ...
        mask = torch.tensor(cv2.resize(mask.cpu().numpy(), (512,512), interpolation=cv2.INTER_NEAREST))
        ...
        args = ResultArgs(
            ...
            class_mask_dict = class_mask_dict
        )
...
def save_result(queue: Queue):
    while True:
        ...
        class_mask_dict = args.class_mask_dict
        ...
        if saver.json_style == 'vipseg':
            ...
            json_out_path = path.join(saver.output_root, saver.json_postfix)

            if not os.path.exists(json_out_path):
                os.makedirs(json_out_path)

            with open(path.join(json_out_path, frame_name[:-4] + '.json'), 'w') as f:

                json.dump(this_annotation, f)
        ...
        if class_mask_dict is not None:
            class_mask_dict_path = path.join(saver.output_root, saver.class_mask_dict_postfix)
            if not os.path.exists(class_mask_dict_path):
                os.makedirs(class_mask_dict_path)
            with open(path.join(class_mask_dict_path, frame_name[:-4] + '.pkl'), 'wb') as f:
        ...
        if save_the_mask:
            ...
            if saver.visualize and saver.object_manager.use_long_id:
                ...
                if prompts is not None:
                    ...
                    all_scores = []
                    for seg in segments_info:
                        if seg["category_id"] is not None:
