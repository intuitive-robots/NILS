...
def _replace_new_line(match) -> str:
    value = match.group(2)
    value = re.sub(r"\n", r"\\n", value)
...
def _parse_json(
    json_str: str, *, parser = parse_partial_json
) -> dict:
...
def parse_json_markdown(
    json_string: str, *, parser = parse_partial_json
) -> dict:
...
def parse_and_check_json_markdown(text: str, expected_keys) -> dict:
    ...
    try:
        ...
        raise json.JSONDecodeError("Invalid JSON output", text, 0)
    for key in expected_keys:
        if key not in json_obj:
            raise json.JSONDecodeError(f"Missing key: {key}", text, 0)
    return json_obj


def parse_json(result, expected_keys,partial: bool = False) -> Any:
    ...
    if partial:
        try:
            ...
            logging.error(f"Invalid json output: {text}")
            return None
    else:
        try:
            return  parse_and_check_json_markdown(text,expected_keys)
